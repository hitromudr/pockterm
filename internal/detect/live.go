...
// been going and how many tokens it has spent, in brackets after whatever it
// calls what it is doing.
//
//	✶ Doing… (1m 13s · ↓ 3.9k tokens)
//	✻ Pondering… (9m 2s · ↑ 31.0k tokens · thought for 17s)
//
// Bracketed, with a duration and the word "tokens" inside — that shape is what
// separates the live line from the one left behind when the turn ends, which is
// the same words in the past tense and no brackets at all:
//
//	✻ Cooked for 19s · 1 shell, 1 monitor still running
//
// The verbs change from release to release ("Pondering", "Doing", "Cooked"), so
// none of them is matched. What does not change is that a turn in flight is
// counting and a turn that is over is not.
var liveCounter = regexp.MustCompile(`(?i)\([^)]*\b\d+s\b[^)]*\btokens?\b[^)]*\)`)
